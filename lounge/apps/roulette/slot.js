...
//
// ボタン活性:
//   IDLE:     START=有効  STOP=無効  RESET=無効
//   SPINNING: START=無効  STOP=有効  RESET=無効
//   STOPPING: START=無効  STOP=無効  RESET=無効
//   RESULT:   START=有効  STOP=無効  RESET=有効
//
// ============================================================
...
(function () {
    ...
    var MAX_MEMBERS = 20;
    var REEL_COPIES = 4;
    var COLOR_CLASSES = ['red', 'blue', 'yellow', 'green', 'purple', 'orange'];
    ...
    function buildReel() {
        ...
        for (var i = 0; i < state.members.length; i++) {
            var colorClass = COLOR_CLASSES[i % COLOR_CLASSES.length];
            symbolsHTML +=
                '<div class="reel-symbol symbol--' + colorClass + '" data-index="' + i + '">' +
                '  <span class="symbol-icon">❼</span>' +
                '  <span class="symbol-name">' + escapeHTML(state.members[i]) + '</span>' +
                '</div>';
        }
    ...
    stopHit.addEventListener('click', function () {
        ...
        console.log('[SLOT] 当選者:', state.members[state.winnerIndex], '(index=' + state.winnerIndex + ')');
    ...
    // ============================================================
    // CRAWL完了後:
    //   1. 少し行き過ぎる（overshoot +半コマ）
    //   2. ±3px のガタガタ揺れ（0.25s）
    //   3. 吸い込み（cubic-bezier で目標位置へ）
    //   4. ポン拡大
    // ============================================================
    function doSlipStop() {
    ...
    // ============================================================
    function showResult() {
        var winnerName = state.members[state.winnerIndex];
